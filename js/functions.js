...
export const showMoreAction = (event) => {
  ...
  for (let z = booksOnPageCount; z < books.length; z++) {
    ...
    bookList[z].addEventListener('click', createDescriptionOverlayFactory(books, authors));
  }
};
 ...
 * @param authors
 */
export function createDescriptionOverlayFactory(books, authors) {
  return function descriptionOverlay(event) {
    event.preventDefault();
    ...
    // show the book summary overlay when it's done being created
...
createDescriptionOverlayFactory(books, authors);
